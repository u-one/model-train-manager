...
### 2.3 品質保証
- 単体テストは Vitest を利用する
- `npm run test:coverage` で Vitest のカバレッジを計測し、HTML レポートを `coverage/index.html` に出力する
- `npm run test:coverage:ui` で watch モードのカバレッジ確認を行う
- GitHub Actions の Coverage ワークフローは Node.js 22 で `npm run test:coverage` を実行し、`coverage/lcov.info` を Codecov にアップロードする
- `src/lib/csv-parser.ts` は CSV の引用符処理、商品/保有車両インポート変換、HTML改行タグ変換を単体テストで検証する

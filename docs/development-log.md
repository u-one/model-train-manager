# 鉄道模型車両管理アプリ 開発記録

## プロジェクト概要
Nゲージ鉄道模型の車両情報と保有状況を管理するWebアプリケーション

## 開発進捗

### CI/CD
- [x] GitHub Actions の Coverage ワークフローを追加し、Vitest カバレッジを Codecov にアップロードする設定を追加
- [x] Coverage ワークフローの Node.js を 22 に固定し、Vitest 設定を ESM として読み込むように変更

### 完了済み
- [x] プロジェクト要件定義（specification.md）
- [x] ディレクトリ構造設計（rules.md）
- [x] UIワイヤーフレーム作成（5画面）
- [x] CLAUDE.md作成
- [x] 既存データ確認（CSV形式）
- [x] 認証方式をGoogle OAuthに簡素化決定

### Phase 1: プロジェクト基盤構築

#### 1.1 Next.jsプロジェクト作成
- [x] `apps/web-next/` 配下にNext.js 15.5.3プロジェクト作成
- [x] TypeScript + Tailwind CSS + ESLint設定
- [x] フォルダ構造整備
- [x] 基本的な型定義ファイル作成

#### 1.2 開発環境セットアップ
- [x] package.json設定（pnpmワークスペース対応）
- [x] 必要ライブラリ導入：
  - [x] @supabase/supabase-js, prisma
  - [x] next-auth（GoogleProviderのみ）
  - [x] lucide-react, @headlessui/react
  - [x] react-hook-form, zod
- [x] 環境変数設定ファイル作成
- [x] 基本的なlib設定ファイル作成

### Phase 2: 基本機能実装

#### 2.1 認証システム（優先実装）
- [x] NextAuth.js Google OAuth設定
- [x] Google Cloud Console設定（本番環境対応）
- [x] ログイン画面（Googleボタンのみ）
- [x] Vercelデプロイ・認証システム動作確認
- [x] ユーザー自動作成機能（signInコールバック）
- [x] AuthGuard認証保護コンポーネント

#### 2.2 データベース環境構築
- [x] Vercel + Supabase統合環境設定
- [x] Prismaスキーマ作成・マイグレーション実行
- [x] データベース接続確認・テストAPI作成
- [x] 本番環境デプロイ確認

#### 2.3 製品情報機能
- [x] 製品一覧画面（検索・フィルタ・ページネーション）
- [x] 製品詳細画面（基本情報・実車情報・保有状況）
- [x] 製品API実装（CRUD操作）
- [x] Vercelデプロイ確認
- [x] 製品追加フォーム画面
- [x] 製品編集フォーム画面

#### 2.4 保有車両機能
- [x] 保有車両一覧画面（検索・フィルタ・認証制御）
- [x] 保有車両詳細画面（詳細情報・整備記録）
- [x] 保有車両API実装（認証付きCRUD操作）
- [x] ダッシュボード更新・ナビゲーション統合
- [x] Vercelデプロイ・エラー修正確認
- [x] 保有車両追加フォーム画面
- [x] 保有車両編集フォーム画面
- [ ] UI改善（色の調整、入力項目の柔軟性向上）

### Phase 2.5: ID/パスワード認証追加
- [x] NextAuth.js Credentials Provider設定
- [x] ユーザー登録フォーム作成
- [x] パスワードハッシュ化機能（bcrypt）
- [x] ログイン画面の認証選択肢追加
- [x] 既存Google認証との併用対応
- [x] UI改善（入力フィールドコンポーネント化）

### Phase 2.6: セット管理機能実装
- [x] セット構成車両管理機能
- [x] 製品間の親子関係（parentCode）設定
- [x] セット詳細画面での構成車両表示
- [x] セット詳細からのセット単品追加機能
- [x] メーカー情報の自動設定
- [x] SetComponent/ParentSetインターフェース統一化
- [x] Next.js 15 params Promise互換性対応

### Phase 2.7: 認証・権限管理の改善
- [x] 製品追加・編集をログイン時のみに制限
- [x] 保有状況をログインユーザーのみ表示
- [x] 保有車両画面の認証必須化
- [x] 認証状態に応じたUI表示制御

### Phase 2.8: 削除機能の追加
- [x] 製品削除機能（確認ダイアログ付き）
- [x] 保有車両削除機能（確認ダイアログ付き）
- [x] 削除権限チェック（所有者のみ）
- [x] 関連データの整合性チェック

### Phase 2.9: 入力項目の柔軟性向上 → Phase 2.14に統合
- [x] Phase 2.14に移行

### Phase 2.10: UI/UX改善 → Phase 2.14に統合
- [x] ダッシュボードのヘッダ重複問題修正
- [x] セット構成車両の一覧表示オプション
- [x] グリッド/リスト表示の切り替え機能
- [x] 未完了項目はPhase 2.14に移行

### Phase 2.11: CSVインポート機能
- [x] CSVパーサー実装（製品情報・保有車両対応）
- [x] CSVプレビュー機能
- [x] インポートAPI実装（/api/products/import, /api/owned-vehicles/import）
- [x] エラーハンドリングとバリデーション
- [x] 日本語CSVフォーマット対応
- [x] HTML改行タグ変換機能
- [x] 大量データ処理対応（592行のテスト完了）
- [x] インポート進捗表示とUI改善

### Phase 2.12: 管理画面・一括削除機能 ✅
- [x] 管理者認証機能・ミドルウェア実装
- [x] 管理画面レイアウト・ダッシュボード作成
- [x] 製品管理画面・一括削除API実装
- [x] 保有車両管理画面・一括削除API実装
- [x] ユーザー管理機能実装
- [x] **全削除機能の実装** - 安全確認付き一括データ削除
- [x] セキュリティ・監査機能の追加

### Phase 2.13: システム改善・修正 ✅
- [x] ページネーション仕様統一（デフォルト100件）
- [x] 製品インポート一意性修正（メーカー+品番対応）
- [x] ページネーションUI視認性向上（色調整）
- [x] .gitignore整備（Claude関連ファイル除外）
- [x] ドキュメント整理・統一（3主要文書の一貫性確保）

### Phase 2.14: Phase2未完了項目の統合実装
#### 2.14.1 インポート機能改善
- [x] セットとセット単品の紐づけ処理（インポート後の自動処理）

#### 2.14.2 UI/UX改善 ✅
- [x] レスポンシブデザインの改善（モバイル最適化）
- [x] 製品一覧・保有車両一覧のデフォルト表示をリスト形式に変更
- [x] フィルタ機能の強化・改善
- [x] 製品一覧でセット単品をデフォルト非表示に設定
- [x] ヘッダに管理者向け管理画面遷移ボタン追加
- [x] 製品一覧の保有数表示をログインユーザーのみに修正

#### 2.14.3 入力項目の柔軟性向上 ✅
- [x] 管理IDの空文字列を許容
  - バリデーション・フォーム・CSVインポート対応完了
  - データベース制約削除（Prisma db push使用）
- [x] 発売年：不明・未設定を許容（既存実装で対応済み）
- [x] 価格：セット単品時のグレーアウト・無効化（製品追加・編集画面）
- [x] 購入日：不明・未設定を許容（既存実装で対応済み）
- [x] 購入情報：必須項目の見直し（全項目任意化完了）
- [x] 価格：未入力・不明を許容（NaN対応完了）
- [x] バリデーションルールの調整（NaN/空文字処理対応）
  - Zodスキーマに`.or(z.nan())`追加
  - フォーム送信時のNaN→undefined変換処理実装
  - TypeScript型エラー修正完了

### Phase 2.15: タグシステム実装 ✅
- [x] 詳細仕様策定 → **[タグシステム仕様書](./tag-system-specification.md)参照**
- [x] データベース拡張（tags, product_tagsテーブル追加）
- [x] API実装（タグCRUD、AND/OR論理演算フィルタ）
- [x] フロントエンド実装（カテゴリ別タグフィルタUI、製品フォーム拡張）
- [x] 製品詳細画面でのタグ表示
- [x] **管理画面でのタグ管理機能** ✅ (2025年1月26日完了)
- [x] **タグ統計・分析機能** ✅ (2025年1月26日完了)
- [ ] 一括タグ付け機能（Phase 3で実装予定）
- [ ] CSVインポート時のタグ自動付与（Phase 3で実装予定）

**Note**: Phase 2.15の主要機能は完了。高度な運用機能はPhase 3で実装予定。

### Phase 2.16: CSVインポート改善・製品未登録時の柔軟な対応 ✅

#### 2.16.1: CSVインポート改善 ✅ (2025年1月24-26日完了)
- [x] 詳細仕様策定 → **[Phase 2.16仕様書](./phase-2.16-csv-import-improvement.md)参照**
- [x] CSVインポート改善（製品未登録時の独立車両自動登録）
- [x] 独立車両情報の自動保存（メーカー・品番・商品名）
- [x] インポート結果の詳細化（製品リンク数・独立車両数の内訳表示）
- [x] インポート結果UI改善（成功件数内訳・注意メッセージ表示）
- [x] 管理画面での独立車両対応（型定義・表示ロジック修正）

#### 2.16.2: 製品情報変換機能 ✅ (2025年1月26日完了)
- [x] 保有車両詳細画面での変換ボタン追加（「この情報で製品を作成」）
- [x] 製品作成フォームの改善（独立車両情報から初期値設定）
- [x] 製品作成後の自動リンク機能（保有車両と製品の紐づけ）
- [x] 独立車両警告表示とガイダンスUI

#### 2.16.3: 一括変換機能（Phase 3で実装予定）
- [ ] 独立車両一覧からの一括変換機能
- [ ] 選択した独立車両から一括で製品情報を作成
- [ ] 同じメーカー・品番の独立車両を1つの製品にまとめる機能

**Note**: Phase 2.16の主要機能（2.16.1-2.16.2）は完了。一括変換機能はPhase 3で実装予定。

### Phase 2.17: 一括操作機能

#### 2.17.1: 製品一括タグ付け機能 ✅ (2025年1月26日完了)
- [x] 製品選択機能（チェックボックス・全選択）
- [x] 一括タグ編集ダイアログ（追加・上書き・削除モード）
- [x] 一括タグ更新API実装
- [x] 進行状況表示とエラーハンドリング

### テスト整備
- [x] `apps/web-next/src/lib/csv-parser.ts` の単体テスト追加
  - `parseCSV` の引用符・エスケープ処理
  - `parseProductCSV` の種別正規化、価格/年変換、HTML改行変換、スキップ行
  - `parseOwnedVehicleCSV` の日付変換、ケース有無変換、備考連結
- [x] Vitest カバレッジ可視化の追加
  - `npm run test:coverage` で text/html/lcov レポートを生成
  - HTML レポートの出力先は `apps/web-next/coverage/index.html`

#### 2.17.2: 条件ベース自動タグ付け
- [ ] 自動タグ付けルール設定（メーカー・品番パターン・キーワード）
- [ ] 管理画面での一括適用機能
- [ ] プレビュー機能（影響製品数表示）
- [ ] 実行ログ表示

#### 2.17.3: 保有車両一括更新機能
- [ ] 保有車両選択機能（チェックボックス）
- [ ] 一括更新ダイアログ（状態・保管条件・備考）
- [ ] 一括更新API実装
- [ ] 部分更新とエラーハンドリング

**詳細**: [Phase 2.17仕様書](./phase-2.17-bulk-operations.md)

### Phase 3: 詳細機能
- [ ] 整備記録機能
- [ ] 画像アップロード機能
- [ ] データエクスポート機能
- [ ] 独立車両一括変換機能（Phase 2.16.3）
- [ ] 一括タグ付け高度機能（CSVインポート時の自動タグ付けルール）

## 技術仕様

### 使用技術
- **フロントエンド**: React (Next.js 15)
- **バックエンド**: Node.js (Next.js API Routes)
- **データベース**: PostgreSQL (Supabase)
- **認証**: NextAuth.js (Google OAuth + ID/パスワード)
- **UI**: Tailwind CSS + Headless UI
- **デプロイ**: Vercel

### データベース設計
主要テーブル:
- products（製品マスタ）
- users（ユーザー）
- owned_vehicles（保有車両）
- maintenance_records（整備記録）

### 既存データ
- `local/車両リスト - 車両商品情報.csv`: 製品マスタデータ
- `local/車両リスト - 保有車両.csv`: 保有車両データ

## 実装順序変更
認証システムを優先実装してデプロイ確認後、DB環境構築を行う方針に変更

## 現在の状況（2025年1月）

### 完成済み機能
- ✅ 認証システム（Google OAuth + ID/パスワード + 自動ユーザー作成）
- ✅ データベース統合（Supabase + Prisma）
- ✅ 製品情報機能（一覧・詳細・検索・フォーム）
- ✅ 保有車両機能（一覧・詳細・認証制御・フォーム）
- ✅ UI改善（入力フィールドコンポーネント化・視認性向上）
- ✅ CSVインポート機能（製品・保有車両データの一括登録、独立車両対応）
- ✅ 管理者機能（権限管理・統計表示・一括削除・全削除）
- ✅ UI/UX改善（フィルタ強化・レスポンシブ対応・セット単品制御）
- ✅ タグシステム（5カテゴリ、AND/OR検索、管理画面）
- ✅ 独立車両システム（CSVインポート時の自動登録、製品情報変換）
- ✅ Vercel本番環境デプロイ

### Phase 2.5実装完了詳細（2025年1月21日）
#### ID/パスワード認証機能
- ✅ **Prismaスキーマ拡張**: Userテーブルにpasswordフィールド追加
- ✅ **bcryptjs導入**: パスワードハッシュ化（12ラウンド）
- ✅ **NextAuth.js拡張**: CredentialsProvider追加
- ✅ **ユーザー登録API**: `/api/auth/register`実装
- ✅ **ログイン画面改修**: ログイン/登録フォーム切り替え機能
- ✅ **認証連携**: Google OAuth + ID/パスワード併用

#### UI改善（コンポーネント化）
- ✅ **Inputコンポーネント**: React Hook Form対応、エラー表示、必須項目表示
- ✅ **Selectコンポーネント**: options配列対応、統一デザイン
- ✅ **TextAreaコンポーネント**: 一貫したスタイリング
- ✅ **視認性向上**: text-gray-900で入力文字の濃度統一
- ✅ **フォーム適用**: signin、products/edit、owned-vehicles/new

#### テスト用アカウント
- **メールアドレス**: test@uoneweb.net
- **パスワード**: password
- **動作確認済み**: ユーザー登録、ログイン、フォーム操作

### Phase 2.6実装完了詳細（2025年1月21日）
#### セット管理機能
- ✅ **parentCode-based セット管理**: 既存Prismaスキーマを活用
- ✅ **セット構成車両API**: `/api/products/[id]/set-components`実装
- ✅ **親セット取得API**: `/api/products/[id]/parent-sets`実装
- ✅ **セット詳細画面拡張**: 構成車両一覧表示、未登録時のメッセージ
- ✅ **セット単品追加機能**: セット詳細から直接追加、親セット品番自動設定
- ✅ **メーカー自動設定**: URLパラメータ経由でメーカー情報も引き継ぎ
- ✅ **インターフェース統一**: SetComponent/ParentSet削除、Product型に統一
- ✅ **Next.js 15対応**: params Promise互換性、Suspense境界対応

### Phase 2.12実装完了詳細（2025年1月23日）
#### 管理者機能・全削除機能
- ✅ **管理者認証システム**: 環境変数`ADMIN_EMAILS`による権限管理
- ✅ **管理画面レイアウト**: AdminLayoutコンポーネント、サイドバーナビゲーション
- ✅ **ダッシュボード**: 統計情報表示（製品数、保有車両数、ユーザー数）
- ✅ **製品管理画面**: ページネーション、検索、一括選択・削除機能
- ✅ **保有車両管理画面**: ユーザー情報付き一覧、メーカー・品番による製品マッチング
- ✅ **ユーザー管理画面**: 全ユーザー一覧、統計情報表示

#### 全削除機能の安全実装
- ✅ **全削除ボタン**: 製品・保有車両管理画面に専用ボタン追加
- ✅ **安全確認ダイアログ**: 「全削除」文字列入力による確認機能
- ✅ **段階的削除処理**: 関連データ（整備記録、実車情報）も含めた一括削除
- ✅ **専用APIエンドポイント**:
  - `/api/admin/products/delete-all` - 全製品削除
  - `/api/admin/owned-vehicles/delete-all` - 全保有車両削除
- ✅ **権限チェック**: 管理者のみアクセス可能、401/403エラーハンドリング
- ✅ **UI/UX改善**: 視覚的警告（濃い赤色）、進行状況表示、操作無効化

#### セキュリティ強化
- ✅ **多段階確認**: 危険な操作には複数の確認ステップ
- ✅ **管理者警告バナー**: 管理画面での注意喚起表示
- ✅ **操作ログ**: コンソールへの削除操作記録
- ✅ **エラーハンドリング**: 適切なエラーメッセージと復旧案内

### Phase 2.13実装完了詳細（2025年1月23日）
#### システム改善・修正
- ✅ **ページネーション仕様統一**: 全画面でデフォルト100件/ページに統一
  - 製品一覧、保有車両一覧、管理画面でのlimit値を100に修正
  - API側とフロントエンド側の両方で対応
- ✅ **製品インポート一意性修正**: メーカー+品番の組み合わせでの重複チェック
  - brand + product_codeでの一意性チェックに変更
  - エラーメッセージも「メーカー 品番」形式に修正
- ✅ **ページネーションUI視認性向上**: 文字色・背景色の改善
  - text-gray-900 font-medium bg-gray-50で明瞭な表示
  - ボタンのdisabled状態も含めて色調整
- ✅ **.gitignore整備**: Claude関連ファイルの適切な除外
  - .claude/, claude_data/, *.claude等の除外ルール追加
  - 開発環境ファイルの包括的な除外設定
- ✅ **ドキュメント整理・統一**: 3主要文書の一貫性確保
  - CLAUDE.md, specification.md, development-log.mdの情報統一
  - Next.js 15、認証方式（Google OAuth + ID/パスワード）の正確な記載

### Phase 2.14.2実装完了詳細（2025年1月23日）
#### UI/UX改善
- ✅ **フィルタ機能強化**: 検索・メーカー・種別フィルタ + アクティブ表示・リセット機能
  - アクティブフィルタの色分けバッジ表示（検索:青、メーカー:緑、種別:紫、セット単品:橙）
  - ワンクリックリセット機能で全フィルタ解除
  - フィルタ状態の視覚的フィードバック強化
- ✅ **セット単品デフォルト非表示**: デフォルトでSET_SINGLE除外、チェックボックスで表示切替
  - excludeSetSingle=trueをデフォルト設定
  - セット単品表示チェックボックスで動的制御
  - ページネーション数の適切な反映（1/6→1/2ページ変動を確認）
- ✅ **レスポンシブデザイン改善**: モバイル最適化レイアウト
  - 375px幅でのフィルタ部分の縦配置最適化
  - フィルタ表示部分のレスポンシブ対応（sm:breakpoint活用）
  - アクティブフィルタ表示の折り返し対応
- ✅ **管理者ボタン追加**: 権限に応じた管理画面アクセス
  - useAdmin hookによる権限チェック
  - /api/admin/status APIでの管理者判定
  - 条件付き管理ボタン表示（test@uoneweb.netで確認済み）
- ✅ **保有数表示修正**: ユーザー固有の保有車両数表示
  - ログインユーザーのIDによる保有車両フィルタリング
  - 非ログイン時も含めた適切な「保有: 0台」表示
  - Prisma LEFT JOINによる効率的な保有数カウント
- ✅ **ビューモードデフォルト変更**: デフォルト表示をリスト形式に設定
  - useViewMode hookでのdefaultValue:'list'設定
  - localStorage永続化による設定保持

#### 技術的修正
- ✅ **Prisma型バリデーション修正**: SET_SINGLEのenum値対応
  - キャッシュクリア（.nextディレクトリ削除）による解決

### Phase 2.15実装完了詳細（2025年1月26日）
#### タグシステム実装

- ✅ **データベース拡張**（2025年1月26日完了）
  - Prismaスキーマに以下のモデル追加:
    - `Tag`: タグマスタテーブル（名前、カテゴリ、表示順）
    - `ProductTag`: 製品・タグ中間テーブル（多対多リレーション）
  - タグカテゴリ: `vehicle_type`, `company`, `feature`, `era`
  - 一意性制約: タグ名はカテゴリ内で一意
  - `prisma db push` でデータベースに反映

- ✅ **タグAPI実装**（2025年1月26日完了）
  - `/api/tags` - タグCRUD操作
    - GET: カテゴリ別タグ一覧、ページネーション対応
    - POST: タグ作成（カテゴリ・名前・表示順）
    - PUT: タグ更新
    - DELETE: タグ削除
  - `/api/tags/categories` - カテゴリ別タグ統計取得
  - `/api/products/:id/tags` - 製品タグ管理
    - GET: 製品に紐づくタグ一覧取得
    - PUT: 製品タグの一括更新（既存削除→新規作成）

- ✅ **製品検索API拡張**（2025年1月26日完了）
  - `/api/products` - タグフィルタ対応
    - パラメータ: `tags` (カンマ区切りタグID), `tag_operator` (AND/OR)
    - AND検索: 指定した全タグを持つ製品のみ
    - OR検索: 指定したいずれかのタグを持つ製品
  - Prisma集約クエリによる効率的なAND検索実装

- ✅ **フロントエンド実装**（2025年1月26日完了）
  - コンポーネント作成:
    - `TagFilter.tsx`: カテゴリ別タグフィルタUI
      - 4カテゴリ（車種、運営会社、特徴・仕様、時代・塗装）表示
      - AND/OR論理演算切り替え
      - カテゴリ展開/折りたたみ機能
      - 選択タグカウント表示
    - `ProductFormTagSelector.tsx`: 製品フォームタグ選択UI
      - カテゴリ別チェックボックス選択
      - 選択済みタグバッジ表示・削除機能
      - グリッドレイアウト

- ✅ **製品一覧ページ統合**（2025年1月26日完了）
  - ファイル: `apps/web-next/src/app/products/page.tsx`
  - TagFilterコンポーネントをサイドバーに追加
  - タグ状態管理（selectedTags, tagOperator）
  - API呼び出し時にタグパラメータ付与
  - アクティブフィルタ表示にタグ情報追加

- ✅ **製品カード・リストアイテムでのタグ表示**（2025年1月26日完了）
  - `ProductCard.tsx`: 最大3タグ表示、残りカウント表示
  - `ProductListItem.tsx`: 最大5タグ表示、残りカウント表示
  - カテゴリ別カラーコーディング:
    - 車種: 青（bg-blue-100 text-blue-800）
    - 運営会社: 緑（bg-green-100 text-green-800）
    - 特徴: 紫（bg-purple-100 text-purple-800）
    - 時代: オレンジ（bg-orange-100 text-orange-800）

- ✅ **製品作成・編集フォームでのタグ選択**（2025年1月26日完了）
  - 製品作成画面 (`apps/web-next/src/app/products/new/page.tsx`)
    - ProductFormTagSelector追加
    - 製品作成後にタグAPIで紐づけ
  - 製品編集画面 (`apps/web-next/src/app/products/[id]/edit/page.tsx`)
    - 既存タグの読み込み（useEffect）
    - タグ更新API呼び出し（onSubmit）
    - ProductFormTagSelector UI追加

- ✅ **製品詳細画面でのタグ表示**（2025年1月26日完了）
  - ファイル: `apps/web-next/src/app/products/[id]/page.tsx`
  - タグセクション追加（説明セクションの下）
  - カテゴリ別カラーバッジ表示
  - API側でproductTags情報を含めるように修正

- ✅ **エラーハンドリング**（2025年1月26日完了）
  - API エラー時の graceful degradation（空配列フォールバック）
  - null チェックとデフォルト値設定
  - TypeScript 型安全性確保

#### 動作確認結果
- ✅ TypeScript型チェック: エラーなし
- ✅ ESLintチェック: エラーなし
- ✅ タグフィルタ機能: 正常動作（4カテゴリ表示確認）
- ✅ 製品作成時のタグ選択: 正常動作
- ✅ 製品編集時のタグ選択: 正常動作
- ✅ 製品詳細でのタグ表示: 正常動作

### Phase 2.15 タグ管理画面実装詳細（2025年1月26日）
#### 管理画面でのタグ管理機能

- ✅ **管理画面ナビゲーション追加**（2025年1月26日完了）
  - ファイル: `apps/web-next/src/components/admin/AdminLayout.tsx`
  - 「タグ管理」メニュー項目追加（`/admin/tags`）
  - Tagsアイコン追加

- ✅ **タグ管理ページ作成**（2025年1月26日完了）
  - ファイル: `apps/web-next/src/app/admin/tags/page.tsx`
  - タグ一覧表示（ID、名前、カテゴリ、説明、使用数、操作）
  - カテゴリフィルタ（5カテゴリ対応）
  - 検索機能（タグ名でフィルタ）
  - タグ作成（モーダル形式）
  - タグ編集（モーダル形式）
  - タグ削除（使用数警告付き確認ダイアログ）
  - ページネーション（100件/ページ）

- ✅ **カテゴリ定数ファイル作成**（2025年1月26日完了）
  - ファイル: `apps/web-next/src/constants/tags.ts`
  - カテゴリ定義の一元管理（Single Source of Truth）
  - カテゴリ別カラー定義
  - ヘルパー関数（getCategoryLabel, getCategoryColor）

- ✅ **カテゴリ見直し・分割**（2025年1月26日完了）
  - 「特徴・仕様」を「商品特徴」と「車両仕様」に分割
  - 最終カテゴリ構成（5つ）:
    - `vehicle_type` (車種) - 青
    - `company` (運営会社) - 緑
    - `product_feature` (商品特徴) - 紫
    - `vehicle_spec` (車両仕様) - インディゴ
    - `era` (時代・塗装) - オレンジ

- ✅ **既存コンポーネントの統一**（2025年1月26日完了）
  - 重複していたカテゴリ定義を削除
  - 定数ファイルをimportするように変更
  - 影響ファイル:
    - `ProductCard.tsx`
    - `ProductListItem.tsx`
    - `ProductFormTagSelector.tsx`
    - `products/[id]/page.tsx`
    - `admin/tags/page.tsx`

#### 動作確認結果
- ✅ TypeScript型チェック: エラーなし
- ✅ ESLintチェック: エラーなし
- ✅ 管理画面でのタグCRUD操作: 正常動作
- ✅ カテゴリフィルタ: 正常動作（5カテゴリ）
- ✅ 使用中タグの削除警告: 正常動作
- ✅ 全画面でカテゴリ定数が統一: 確認済み

### Phase 2.15 タグ統計・分析機能実装詳細（2025年1月26日）
#### 既存データへのタグ適用・運用改善

- ✅ **管理ダッシュボードにタグ統計追加**（2025年1月26日完了）
  - ファイル: `apps/web-next/src/app/admin/page.tsx`
  - 新規セクション「タグ統計」を追加
  - 表示項目:
    - 総タグ数
    - タグ付き製品数
    - タグ付与率（パーセント表示、自動計算）
  - クイックアクションにタグ管理へのリンク追加

- ✅ **Admin Stats API拡張**（2025年1月26日完了）
  - ファイル: `apps/web-next/src/app/api/admin/stats/route.ts`
  - 統計情報に以下を追加:
    - `totalTags`: 総タグ数（prisma.tag.count()）
    - `taggedProducts`: タグが付いている製品数
  - 並行取得でパフォーマンス最適化
  - Promise.allで既存統計と同時取得

- ✅ **タグ管理機能の完成**（2025年1月26日完了）
  - タグのCRUD操作
  - カテゴリ別フィルタ
  - 使用数表示
  - 統計情報表示
  - 一元管理されたカテゴリ定義

#### 今後の実装予定
- ⏸️ **一括タグ付け機能** - Phase 3で実装
  - 製品管理画面での複数選択
  - 選択した製品に一括でタグを付与
- ⏸️ **CSVインポート時のタグ自動付与** - Phase 3で実装
  - インポート時に製品名やメーカーから自動的にタグを提案
  - ルールベースの自動タグ付け

#### 動作確認結果
- ✅ TypeScript型チェック: エラーなし
- ✅ ESLintチェック: エラーなし
- ✅ タグ統計表示: 正常動作
- ✅ タグ付与率計算: 正常動作
- ✅ クイックアクションリンク: 正常動作

### Phase 2.14.3実装詳細（2025年1月24日）
#### 入力項目の柔軟性向上
- ✅ **管理IDの空文字列許容**（2025年1月24日完了）
  - バリデーション: `z.string().optional().default('')` に変更
  - フォーム: 追加・編集画面から必須マーク削除、プレースホルダー更新
  - CSVインポート: 空文字列の場合の重複チェックをスキップ
  - データベース制約削除: `owned_vehicles_user_id_management_id_key` を削除
    - `prisma db pull` で現在のDB状態を取得
    - Prismaスキーマから `@@unique([userId, managementId])` を削除
    - `prisma db push` でデータベースに反映
  - **重要**: Prismaスキーマを Single Source of Truth として管理
  - ドキュメント追加: [データベーススキーマ管理ガイド](./database-schema-management.md)

- ✅ **発売年の不明・未設定許容**（既存実装で対応済み）
  - データベース: `releaseYear Int?` - NULL許可
  - バリデーション: すでに `.optional()` で任意項目
  - フォーム: 必須マーク（*）なし、既に任意扱い

- ✅ **価格のセット単品時グレーアウト・無効化**（2025年1月24日完了）
  - 製品追加・編集画面: `productType === 'SET_SINGLE'` の場合
    - 価格入力フィールドに `disabled={isSetSingle}` 属性追加
    - グレーアウトスタイル: `bg-gray-100 text-gray-500 cursor-not-allowed`
    - プレースホルダー: 「親セット価格に含まれます」
    - 情報メッセージ: 「ℹ️ セット単品の価格は親セットに含まれるため、入力不要です」
  - タイプ選択に応じた動的UI制御（`form.watch('type')`）

- ✅ **購入日の不明・未設定許容**（既存実装で対応済み）
  - データベース: `purchaseDate DateTime?` - NULL許可
  - バリデーション: `z.string().optional()` で任意項目
  - フォーム: 必須マーク（*）なし、任意扱い

- ✅ **購入情報の必須項目見直し**（既存実装で対応済み）
  - 購入価格（税抜・税込）: `.optional()` で任意項目
  - 購入店: `z.string().optional()` で任意項目
  - 購入状態: `z.enum(['NEW', 'USED']).optional().or(z.literal(''))` で空文字許容

- ✅ **価格の未入力・不明許容 + NaN対応**（2025年1月24日完了）
  - バリデーション修正:
    ```typescript
    // product.ts & owned-vehicle.ts
    priceExcludingTax: z.number().min(0, '価格は0以上で入力してください').optional().or(z.nan())
    priceIncludingTax: z.number().min(0, '価格は0以上で入力してください').optional().or(z.nan())
    releaseYear: z.number().int().min(1900).max(2030).optional().or(z.nan())
    purchaseCondition: z.enum(['NEW', 'USED']).optional().or(z.literal(''))
    ```
  - フォーム送信処理修正（全4ファイル）:
    - `apps/web-next/src/app/products/new/page.tsx`
    - `apps/web-next/src/app/products/[id]/edit/page.tsx`
    - `apps/web-next/src/app/owned-vehicles/new/page.tsx`
    - `apps/web-next/src/app/owned-vehicles/[id]/edit/page.tsx`
    ```typescript
    // NaN判定による変換処理
    releaseYear: isNaN(data.releaseYear as number) ? undefined : data.releaseYear
    priceExcludingTax: isNaN(data.priceExcludingTax as number) ? undefined : data.priceExcludingTax
    priceIncludingTax: isNaN(data.priceIncludingTax as number) ? undefined : data.priceIncludingTax
    purchaseCondition: data.purchaseCondition === '' ? undefined : data.purchaseCondition
    ```
  - react-hook-formの`valueAsNumber`がempty時に`NaN`を返す問題に対応
  - TypeScript型エラー修正完了（型チェック・ビルド成功）

- ✅ **バリデーションルール総合調整**（2025年1月24日完了）
  - `.or(z.nan())`により、NaN値も許容するスキーマに変更
  - `.or(z.literal(''))`により、空文字列も許容（enum項目）
  - フォーム送信時に`isNaN()`判定でundefinedに変換
  - 空フォーム送信時のバリデーションエラー解消
  - 必須項目は`vehicleType`, `currentStatus`, `storageCondition`のみ

#### 動作確認結果
- ✅ TypeScript型チェック: `npm run type-check` - エラーなし
- ✅ Next.jsビルド: `npx next build --turbopack` - 成功（30ルート生成確認）
- ✅ 全フォームで空欄提出時のバリデーションエラー解消

### Phase 2.16.1実装完了詳細（2025年1月24日）
#### CSVインポート改善・独立車両自動登録

- ✅ **CSVインポートAPI修正**（2025年1月24日完了）
  - ファイル: `apps/web-next/src/app/api/owned-vehicles/import/route.ts`
  - 製品が見つからない場合、**エラーにせず独立車両として登録**
  - メーカー・品番・商品名を`independent_vehicles`テーブルに自動保存
  - `is_independent`フラグをtrueに設定
  - 独立車両情報の説明文を自動生成
  ```typescript
  independentVehicleData = {
    brand: vehicleData.productBrand,
    productCode: vehicleData.productCode,
    name: vehicleData.productName || `${vehicleData.productBrand} ${vehicleData.productCode}`,
    description: `CSVインポート時に製品が見つからなかったため独立車両として登録...`
  }
  ```

- ✅ **インポート結果の詳細化**（2025年1月24日完了）
  - `linkedCount`: 製品リンク数の集計
  - `independentCount`: 独立車両数の集計
  - APIレスポンス形式拡張:
  ```json
  {
    "totalRows": 150,
    "successCount": 145,
    "linkedCount": 120,      // 製品リンク成功
    "independentCount": 25,  // 独立車両として登録
    "errorCount": 5,
    "errors": [...]
  }
  ```

- ✅ **インポート結果UI改善**（2025年1月24日完了）
  - ファイル: `apps/web-next/src/components/CSVImport.tsx`
  - 成功件数の内訳表示（📦製品リンク、📝独立車両）
  - 独立車両登録時の注意メッセージ表示
  - 視覚的なアイコンとカラーコーディング
  - 「⚠️ 注意: X件が独立車両として登録されました。保有車両一覧から製品情報を作成できます。」

- ✅ **管理画面での独立車両対応**（2025年1月24日完了）
  - ファイル: `apps/web-next/src/app/admin/owned-vehicles/page.tsx`
  - 型定義の修正:
    - `productId: number | null` - null許容
    - `isIndependent: boolean` - 独立車両フラグ追加
    - `product: {...} | null` - productオブジェクトnull許容
  - 表示ロジックの修正:
    - メーカー: `vehicle.product?.brand || (vehicle.isIndependent ? '(独立記録)' : '-')`
    - 品番: `vehicle.product?.productCode || '-'`
    - 商品名: `vehicle.product?.name || (vehicle.isIndependent ? '(独立記録車両)' : '-')`
  - エラー修正: `Cannot read properties of null (reading 'brand')` 解消

- ✅ **独立車両フォーム修正**（2025年1月24日完了）
  - ファイル:
    - `apps/web-next/src/lib/validations/owned-vehicle.ts`
    - `apps/web-next/src/app/owned-vehicles/new/page.tsx`
    - `apps/web-next/src/app/owned-vehicles/[id]/edit/page.tsx`
  - `productId`バリデーション修正: `.or(z.nan())` 追加
  - フォーム送信処理: `isNaN(data.productId as number)` 判定追加
  - 独立記録選択時のバリデーションエラー解消

#### 動作確認結果
- ✅ TypeScript型チェック: エラーなし
- ✅ 管理画面での独立車両表示: 正常
- ✅ CSVインポートで製品未登録時の独立車両登録: 動作確認済み
- ✅ インポート結果の内訳表示: 正常

### Phase 2.17.1実装完了詳細（2025年1月26日）
#### 製品一括タグ付け機能

- ✅ **一括タグ更新API**: `/api/products/bulk-update-tags`実装
  - ファイル: `apps/web-next/src/app/api/products/bulk-update-tags/route.ts`
  - 3つのモード対応（add/replace/remove）
  - トランザクション処理による安全な一括更新
  - バリデーションとエラーハンドリング

- ✅ **BulkTagEditDialogコンポーネント**: 一括タグ編集ダイアログ
  - ファイル: `apps/web-next/src/components/BulkTagEditDialog.tsx`
  - モード選択UI（追加・上書き・削除）
  - カテゴリ別タグ選択（ProductFormTagSelectorと同様のUI）
  - 選択タグのバッジ表示
  - 適用内容のプレビュー表示
  - 確認ダイアログ

- ✅ **製品一覧画面の拡張**: チェックボックス選択機能追加
  - ファイル: `apps/web-next/src/app/products/page.tsx`
  - チェックボックス選択（個別・全選択）
  - 選択状態管理（Set型）
  - 一括操作ツールバー（選択件数表示・一括タグ編集ボタン）
  - グリッドモード・リストモード両対応

#### 動作確認結果
- ✅ TypeScript型チェック: エラーなし
- ✅ Next.jsビルド: 成功
- ✅ ESLint警告: 新規追加コードに警告なし

### Phase 2.17.1 + α: カテゴリ別「なし」オプション実装 ✅ (2025年1月26日)
#### タグフィルタ機能強化

- ✅ **カテゴリ別「なし」オプション追加**（2025年1月26日完了）
  - 各タグカテゴリごとに「なし」選択可能
  - 車種・運営会社・商品特徴・車両仕様・時代塗装の5カテゴリ対応
  - 「なし」選択時は該当カテゴリのタグが未設定の製品のみ表示

- ✅ **API拡張**: `/api/products` - カテゴリ別「なし」条件対応
  - パラメータ: `no_tags_categories` (カンマ区切りカテゴリ名)
  - Prisma NOT条件による効率的な除外検索実装
  - 通常のタグフィルタと組み合わせ可能

- ✅ **TagFilterコンポーネント拡張**: カテゴリ別「なし」UI
  - ファイル: `apps/web-next/src/components/TagFilter.tsx`
  - 各カテゴリに「なし」チェックボックス追加（黄色バッジ表示）
  - 「なし」選択時は該当カテゴリのタグ選択を無効化
  - 説明文「（カテゴリが未設定の製品）」を追加
  - ヘルプテキスト更新

- ✅ **製品一覧・保有車両一覧画面対応**
  - `noTagsCategories`ステート管理追加
  - フィルタリセット時の「なし」条件クリア対応
  - 依存配列更新による適切な再描画制御

#### 使用例
- **車種：なし** → 車種タグが付いていない製品のみ表示
- **運営会社：なし + 車種：電車** → 運営会社未設定かつ車種が電車の製品
- 複数カテゴリで「なし」の同時選択も可能

#### 動作確認結果
- ✅ TypeScript型チェック: エラーなし
- ✅ カテゴリ別「なし」フィルタ: 正常動作
- ✅ タグ選択との排他制御: 正常動作
- ✅ フィルタリセット: 正常動作

### 次の実装予定

#### Phase 2.17.2 - 条件ベース自動タグ付け
1. **自動タグ付けルール設定** - メーカー・品番パターン・キーワードによる自動タグ付け
2. **管理画面での一括適用** - プレビュー機能・実行ログ表示

#### Phase 2.17.3 - 保有車両一括更新
1. **保有車両一括更新** - 状態・保管条件・備考の一括変更

### 実装方針
- **一画面ずつ段階的に実装**
- **react-hook-form + zod によるバリデーション**
- **作成→動作確認→デプロイのサイクル**



# 完了済み機能・修正事項

## 追加・編集画面全般
- ✅ テキストの色が薄くてよく見えない → コンポーネント化で解決
  - ✅ h2やselectアイテム → text-gray-900で統一

## ダッシュボード
- ✅ ヘッダが二重に表示されている
- ✅ グリッドでなく一覧でも表示したい

## 製品一覧
- ✅ 製品追加・編集はログイン時のみ可能とする

## 製品詳細
### セット構成車両
- ✅ グリッドでなく一覧で表示したい

### 保有状況
- ✅ ログイン時のみ表示
- ✅ 表示するのはログインしているユーザーの情報のみ

## 保有車両一覧
- ✅ この画面はログインしたユーザだけが表示可能

### 管理ID
- ✅ 必須項目の見直し

## ページネーション
- ✅ デフォルト100件に変更

## インポート
- ✅ 製品のインポートの一意性はメーカと品番に変更
- ✅ 保有車両と製品の紐づけは、メーカーと品番で紐づけするように変更

## 管理機能
- ✅ 削除機能がないので追加が必要
- ✅ 管理ページとデータの一括削除機能追加

**注意**: 上記メモ項目は適切なPhase（2.9〜2.14、3）に取り込み済みです。


# メモ
- UIの"SET_SINGLE"などの表示は「セット単品」など日本語表示
  - むしろDBやコード上の取り扱いをSET_SINGLEなどにすべき

- CSVインポートと管理は、一番上のヘッダ部分に移動

# ソート順の追加
## 保有車両一覧
  - ソート種別
    - 購入日+登録順
    - 名称
    - 管理ID
    - 分類順
  - 降順・昇順
  - デフォルト：購入日 降順
## 製品一覧
  - ソート種別
    - 登録順
    - 名称
    - メーカー＋品番
    - 分類順
  - 降順・昇順
  - デフォルト：登録順 昇順



## 保有車両
- 登録が困難
  - メーカー選んだら候補を絞り込みが必要
  - 品番と選択するときに名称と一緒に表示、従来方式のselect項目を廃止

## 製品情報
- メーカ・品番・商品名を主題に
- 購入サイトではないので、価格情報は補足的な情報であり目立たせなくて良い

## 製品情報
- 同じ品番でもバリエーションが存在する場合があり、その考慮を入れたい
  - 例1：名称が変わり、国鉄 新幹線 、JR 新幹線 が混在
  - 例2：異なる製造年による微妙なロット違い

## その他
- リストで管理画面のような一行表示を選択可能にする
- 1ページあたりの件数を変更可能にする
- 管理用の製品管理画面を製品一覧に統合


  

## リファクタリング考慮
- 各page.tsxにある interface ownedVehicleの定義重複してて問題ない？ちょっとずつ違いそうだが
- api/owned-vehicles/[id]/route.ts のPUTのif文の分岐がちょっとアヤシイ 
- api/owned-vehicles/の各route.ts (import含む)は共通化・一貫性観点などで見直したほうが良さそう
- independentVehicleとproductが別々に存在しているのがちょっと微妙 productの方に抽象概念まとめられないか。（
